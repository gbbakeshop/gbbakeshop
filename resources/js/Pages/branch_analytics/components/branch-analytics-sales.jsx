...
export default function BranchAnalyticsSales({ period }) {
    ...
    // useEffect(() => {
    //     get_branch_period(period,path).then((res) => {
    //         console.log('data',res.records)
    //        setData({
    //           ...data,
    //           sales:res.records.reduce((sum, record) => sum + record.total_sales, 0),
    //           charges:res.charges.reduce((sum, record) => sum + parseFloat(record.total_charge), 0),
    //           expenses:res.expenses.reduce((sum, record) => sum + record.total_expenses, 0)
    //         })
    //     });
    // }, [period]);
